...
public class Fallable {
     ...
     */
    public void fall() {
        if (Map.getInstance().getMap()[entity.x][entity.y + 1].getSprite() == 'v') {
            Map.getInstance().getMap()[entity.x][entity.y] = new Empty(entity.x, entity.y, entity.map);
            entity.y++;
            setFalling(true);
            Map.getInstance().getMap()[entity.x][entity.y] = entity;
        } else {
            try {
                Map.getInstance().getMap()[entity.x][entity.y].fallable.setFalling(false);
            } catch (final Exception e) {
                System.out.print("");
            }
        }
        if (Map.getInstance().getMap()[entity.x][entity.y + 1].getSprite() == 'p') {
            try {
                if (Map.getInstance().getMap()[entity.x][entity.y].fallable.falling) {
        ...
        if (Map.getInstance().getMap()[entity.x][entity.y + 1].getSprite() == 'm') {
            for (int i = 2; i >= 0; i--) {
                for (int j = 2; j >= 0; j--) {
